...
type PaymentOption interface {
	ProcessPayment(account string, amount float32) bool
}
...
func (c *CreditCard) ProcessPayment(account string, amount float32) bool {
	fmt.Println("Processing a credit card payment...")
	fmt.Println("%s is paying %f", c.OwnerName(), amount)
	return true
}
